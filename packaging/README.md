...
| **Flathub** | [`../flatpak/`](../flatpak) | Flathub review PR | needs a committed screenshot (see below) |
...
1. Commit a real screenshot and point `<screenshot>` in the metainfo at its raw
   GitHub URL. `docs/screenshot.png` is generated by
   `packaging/screenshot.sh` — the offline renderer drawing a synthetic test
   tone, so it's genuinely the app's own output.
