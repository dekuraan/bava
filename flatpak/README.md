...
| `io.github.dekuraan.bava.svg` | App icon — **generated** by `packaging/icon/gen-icons.py` (shared source with the macOS `.icns`; see `packaging/icon/README.md`) |
...
- **Before Flathub submission:** the screenshot the metainfo points at is
  `docs/screenshot.png`, regenerated by `packaging/screenshot.sh` (bava's own
  offline renderer drawing a synthesised test tone). What's left is hosting this
  manifest in a `flathub/io.github.dekuraan.bava` repo — see
  [`../packaging/README.md`](../packaging/README.md#flathub) for the submission
  steps. The app-id assumes you own the `github.com/dekuraan/bava` repo
  (required for the `io.github.*` namespace).
